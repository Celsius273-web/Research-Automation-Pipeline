...
Domain: Optimization. Starting subfield: Bayesian Optimization. This subfield has the most standardized tooling (BoTorch, Ax, GPyOpt, scikit-optimize) and benchmark suites (BBOB, COCO), which makes results tractable to compare against published numbers.

Filter out any paper that pairs a new optimizer with large-scale deep learning training. Those need GPU hours this hardware cannot provide. Widen the keyword set to the rest of black-box optimization only after the pipeline works end to end on Bayesian Optimization alone.
